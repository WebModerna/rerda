...
class HomeModalWindow extends Module
{
	...
	public function _displayForm(){
		...
		// Agregado por mi para evitar el mensaje de variable undefined
	...
	<div style="max-width: 95%;">
		...
		<hr/>
		<form action="'.$_SERVER['REQUEST_URI'].'" method="post" style="max-width:95%;">
		...
		foreach ($lngs as $lng){
			$lng = (object)$lng;
			$t .= '
			<h4 onclick="openCloseLayer(\'lang'.$lng->id_lang.'-f\')" style="cursor:pointer; background-color:#fec; padding:10px;" >'
			.$lng->name.' '.$this->l('Content ').' <img src="../img/admin/add.gif" /></h4>
			<div id="lang'.$lng->id_lang.'-f"  style="display:none">
			<textarea rows="1" name="form_'.$lng->id_lang.'" class="rte" id="form_'.$lng->id_lang.'" >';
	...
	function hookHome($params)
	{
		...
		global $smarty;
		$dontshowhmwlink = '<br/><a class="button" href="?dontshowhmw=yes"><span class="icon-remove"> </span> <small>'.$this->l(' Dont Show This Again ').'</small></a><style>.fancybox-wrap .fancybox-skin .fancybox-inner { height: auto !important; }</style>';
